program main
  ...
  ! build multifab with 1 component and 3 ghost cells
  call multifab_build(phio,la,1,3)
  call multifab_build(phin,la,1,3)
  
  ! initialize phi
  call init_phi(phio,dx,prob_lo)
  ...
  if (plot_int .gt. 0) then
     call write_plotfile(la,phio,istep,dx,time,prob_lo,prob_hi)
  end if
  ...
  do istep=1,nsteps
     ...
     if (plot_int .gt. 0) then
        if (mod(istep,plot_int) .eq. 0 .or. istep .eq. nsteps) then
           ! write out plotfile
           call write_plotfile(la,phin,istep,dx,time,prob_lo,prob_hi)
        end if
     end if

     call multifab_copy_c(phio,1,phin,1,1,3)
