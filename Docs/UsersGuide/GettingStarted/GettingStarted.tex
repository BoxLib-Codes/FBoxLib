We now give an overview of common data structures used in \FBoxLib, followed by a simple
example that makes use of these structures.
...
\FBoxLib\ contains the most fundamental objects used to construct parallel
...
the coarsest (base) level of refinement 
(called level $\ell=1$ in Fortran90) and can be represented on one
grid or divided into many grids.
Higher levels of refinement have cells that are finer by a ``refinement ratio''
of 2 (in Fortran90).  The grids are properly nested in the sense that the union 
...
corners.  In \FBoxLib, data that is on an face is termed `nodal'
...
\FBoxLib\ uses $0$-based spatial indexing, and for data that is nodal in one or more direction,
the integer index corresponds to the lower boundary in that direction (see Figure~\ref{fig:dataloc}).
In our \FBoxLib\ applications, the state data (velocity, density, 
...
Also note that \FBoxLib\ uses $0$-based indexing, e.g., in each of these centerings, 
...
\begin{itemize}
\item In Fortran90 \FBoxLib, we build dimension-independent code at compile-time, 
...
\end{lstlisting}
This is rather cumbersome, so instead we use built-in \FBoxLib\ functions to
...
In \FBoxLib, we don't usually deal with 
...
To build a \MultiFab, we require a \layout\ (in Fortran90).
...
{\tt FBoxLib/Tutorials/GettingStarted}.
...
This contains compiler settings and directories required by the make system to build the code.
  ...
  \begin{itemize}
    ...
    Change this to point to the \FBoxLib\ home directory.  Alternatively, you can define {\tt BOXLIB\_HOME}
...
Source code that is not within the {\tt FBoxLib/Src/} tree.  Note that 
if a file that exists in the
{\tt FBoxLib/Src/} tree also exists in the local directory, the local copy takes precedence
as long as the {\tt GNUmakefile} lists your local directory as a {\tt VPATH\_LOCATIONS} 
before the \FBoxLib\ source code directory, {\tt FBoxLib/Src/F\_BaseLib}.
...
\lstinputlisting[backgroundcolor=\color{light-green}]{../../Tutorials/GettingStarted/main.f90}
and {\tt work\_on\_data.f90}:
\lstinputlisting[backgroundcolor=\color{light-green}]{../../Tutorials/GettingStarted/work_on_data.f90}

The {\tt multifab\_fill\_boundary} function
fills all ghost cells on periodic domain boundaries, as well as interior
...
Refer to {\tt FBoxLib/Src/F\_BaseLib/multifab\_f.f90} or 
{\tt FBoxLib/Src/C\_BaseLib/MultiFab.H} for a complete listing.
...
\begin{itemize}
\item The Fortran90 version of this example is contained in {\tt FBoxLib/Tutorials/HeatEquation\_EX1}.
\end{itemize}
...
\begin{lstlisting}[backgroundcolor=\color{light-red}].
~/FBoxLib/Tutorials/HeatEquation_EX1> ls -1 plt*/Header | tee movie.visit
plt00000/Header
plt01000/Header
plt02000/Header
plt03000/Header
...
We will now demonstrate how to run the example in {\tt FBoxLib/Tutorials/HeatEquation\_EX1}
using MPI parallelism.
On your local machine, if you have MPI installed, you must first configure
{\tt FBoxLib/Tools/F\_mk/GMakeMPI.mak} and {\tt FBoxLib/Tools/C\_mk/Make.mpi}
...
To run in parallel on the edison machine at NERSC, first copy the \FBoxLib\ source code
into your home directory on edison and go to the {\tt HeatEquation\_EX1} directory.
...
The first heat equation tutorial 
supports hybrid MPI/OpenMP parallelism for {\bf three-dimensional problems only}.
You may add OMP parallelism to two-dimensional work loops, but be advised that subroutines within the \FBoxLib\ infrastructure 
...
set {\tt OMP=t} (in Fortran) in the {\tt GNUmakefile}.
